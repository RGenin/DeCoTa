...
if __name__ == '__main__':
    ...
    if args.net_resume:
        ...
        net.load_state_dict(            # net correspond au réseau qui fait de l'UDA
            torch.load(pretrain_src_checkpoint, map_location=torch.device('cpu'))
        )
        twin.load_state_dict(           # twin correspond au réseau qui fait le SSL
    ...
    def train():
        ...
        len_train_target = len(target_loader)      # donne le nombre de batchs dans le loader
        ...
        for step in range(args.start, all_step): # itération pas sur les epochs mais sur les batchs
            ...
            out_1 = net(data)       # sortie de net sur les images de la cible
            ...
            im_u_1 = im_data_tu[u_2_mask]   # récupération des images dont les labels ont été prédis par net avec une probalité supérieure au seuil (th, par défaut : 0.5)
            psl_u_1 = u_2_pred[1][u_2_mask] # récupération de l'étiquette des images dont les labels ont été prédis par net avec confiance > seuil
            im_u_2 = im_data_tu[u_1_mask]   # idem pour le réseau twin
            ...
            # stream 1               # UDA
            if im_u_1.size(0) > 0:   # si il y au moins une image qui a eu une prédiction confiante sur net

                size_1 = im_u_1.size(0) # nombre d'images qui ont une prédiction confiante sur net
                # print('stream 1: {}'.format(size_1))

                t_idx = torch.randperm(im_data_t.size(0))[0:size_1] # génère une liste d'indice aléatoire de la taille du nombre d'images dans les targets. Puis en séléctionne le nombre d'images qui ont eu une confiance importante par Net
                mixed_x = lam * im_data_t[t_idx] + (1-lam) * im_u_1 # MixUp entre t_idx (c'est un chiffre/nombre) images provenant de target et t_idx image provenant des images non étiquetées auxquelles nous venons d'ajouter une étiquette
                y_a, y_b = gt_labels_t[t_idx], psl_u_1  # y_a => vrais labels, y_b => pseudos labels

                out_mix = net(mixed_x)  # prédictions des étiquettes avec comme jeu de données les images ayant servies au MixUp
                loss_mix_1 = lam * criterion(out_mix, y_a) + (1-lam) * criterion(out_mix, y_b) # pertes associées

                loss_1 += loss_mix_1 # ajout de la perte sur MixUp à la perte globale sur target par net (UDA)
                zero_grad_all()
            ...
            zero_grad_all()
            # stream 2              # Idem que vue avant mais sur SSL
            if im_u_2.size(0) > 0:
    ...
    def test_ensemble(loader): # teste le modèle "vanilla-ensemble" : deux MIST (qui se base sur DS, DT, DU mais qui mette à jour leur ensemble de train eux même sur chacun) et font une moyenne de leurs prédictions
        net.eval()
        twin.eval()
        correct = 0
        correct_test_1 = 0
